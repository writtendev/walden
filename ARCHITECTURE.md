...
**conditional put** of `tx/<seq>.json`, conditioned on that key not already
existing (`If-None-Match: *`). A stale writer — a process presumed dead that
isn't, a second instance started by mistake — gets back HTTP 412: definitive
proof that another writer has already appended at that sequence (spec
§11.4). It stops serving writes for that stream — not the instance —
immediately. It never guesses: there is no previous-sequence header to
recompute, so there is nothing to retry, and re-reading the head is
forbidden.
...
on the host still boots journal-less. `--journal` given an empty value is
refused, naming the flag, because `fs.Visit` proves the operator typed it —
booting journal-less silently on an explicit act would break walden's first
promise. A whitespace-only value is refused too, from either source, but by
the check the two paths share, which reports only that the value is
whitespace and names neither `--journal` nor `WALDEN_JOURNAL`; that is still
a failed substitution, not a decision to run without a journal, so it is
refused rather than treated as unset.
...
| repository on disk without walden's hook     | the next push repairs the symlink at `hooks/pre-receive` and logs it if it displaced another target; anything else there — a regular file, a directory — is left untouched and that push refuses in one line, as does a repair that cannot be written — reads keep serving either way | move the foreign hook aside; the next push installs walden's |
