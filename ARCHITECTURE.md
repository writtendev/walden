...
Every git child walden execs — `upload-pack`, `receive-pack`, the ref
advertisement, `git init` — runs with system and global git config pinned off
...
`/etc/gitconfig` or the server user's `~/.gitconfig`.
