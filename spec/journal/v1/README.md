...
timestamp:<timestamp>\n
```
Each line terminates with a newline (`\n`, `0x0A`). `<seq>` is the decimal sequence number with no leading zeros, unchanged by the encoding rule of section 1.1. `token_hash` is written verbatim, with no lowercasing, and `token_id` and `timestamp` are likewise written exactly as the record carries them — see section 4.3's payload note for why `token_hash` is not folded the way `CanonicalMarkerPayload` and `CanonicalRefUpdatePayload` fold an OID.
...
2. **A record whose signature does not verify never reaches the table.** `token_create` and `token_revoke` are verified against the key active at their own meta sequence (section 4.3's payload note) before section 8's replay applies either to the table. A missing, malformed, or mismatched signature is refused outright (section 8.1, rule 19); the reader stops rather than skipping the record or guessing at what it would have granted.
